...
export const BarcodeScanner: React.FC<BarcodeScannerProps> = ({ onScan, onClose }) => {
  ...
  // Ref pour bloquer les scans multiples immédiatement
  ...
  const startScan = () => {
    ...
    // Fix TS2774: getUserMedia is always defined in modern TS envs, just check mediaDevices
    if (navigator.mediaDevices) {
      if (videoRef.current) {
        codeReader.current.decodeFromVideoDevice(
            null, // Fix TS2345: Must be null, not undefined
            videoRef.current, 
            (result, err) => {
