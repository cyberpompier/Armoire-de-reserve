...
import { ShieldAlert, Package, Activity, Sparkles, ArrowUpRight, ArrowDownLeft, Clock, HardHat, Shirt, Footprints, Hand, Layers } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ state }) => {
  ...
  // Calcul du stock disponible par type
  const uniqueTypes = Array.from(new Set(state.inventory.map(i => i.type)));
  const stockByType = uniqueTypes.reduce((acc, type) => {
    acc[type] = state.inventory.filter(i => i.type === type && i.status === EquipmentStatus.AVAILABLE).length;
    return acc;
  }, {} as Record<string, number>);

  const getIconForType = (type: string) => {
    const t = type.toLowerCase();
    if (t.includes('casque')) return <HardHat className="w-4 h-4" />;
    if (t.includes('veste') || t.includes('parka') || t.includes('blouson') || t.includes('textile')) return <Shirt className="w-4 h-4" />;
    if (t.includes('botte') || t.includes('ranger') || t.includes('chaussure')) return <Footprints className="w-4 h-4" />;
    if (t.includes('gant')) return <Hand className="w-4 h-4" />;
  ...
  return (
    <div className="p-4 pb-24 space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2 overflow-x-auto pb-1 -mx-1 px-1 scrollbar-hide">
          <div className="flex gap-2">
            {Object.entries(stockByType).sort((a, b) => b[1] - a[1]).map(([type, count]) => (
              <div key={type} className={`flex items-center gap-2 px-3 py-2 rounded-xl border shrink-0 min-w-[100px] ${count > 0 ? 'bg-white border-slate-200 shadow-sm' : 'bg-slate-50 border-slate-100 opacity-60'}`}>
                <div className={`p-1.5 rounded-lg ${count > 0 ? 'bg-slate-50 text-fire-600' : 'bg-slate-100 text-slate-400'}`}>
                  {getIconForType(type)}
                </div>
                <div className="flex flex-col">
                  <span className="text-[10px] font-bold text-slate-500 uppercase leading-none mb-1 truncate max-w-[80px]">{type}</span>
                  <span className="text-sm font-bold text-slate-800 leading-none">{count}</span>
                </div>
              </div>
            ))}
          </div>
